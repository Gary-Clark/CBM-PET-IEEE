...
// loop for file name
    ...
    while (EOI_ == HIGH){
      ...
      file_name[index] = (~PINC);
//      Serial.print(char(file_name[index]));
      if (file_name[1] == ':'){                 // if filename is preceeded by drive number, strip it out
        drive = char(file_name[0]);             // as it is not used
        index= -1;
        file_name[1] = '0';
      }
      index=index+1;
